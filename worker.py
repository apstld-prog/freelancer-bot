...
log = logging.getLogger("worker"); logging.basicConfig(level=os.getenv("LOG_LEVEL","INFO"))
UTC = timezone.utc
...
try: FX_RATES = json.loads(os.getenv("FX_RATES","")) if os.getenv("FX_RATES") else _DEFAULT_FX
except Exception: FX_RATES = _DEFAULT_FX
...
def _kws(db,u):
    ...
    try:
        ...
        if rel is not None:
            for k in list(rel):
                out.append((getattr(k,"keyword",None) or getattr(k,"text",None) or "").strip())
            return [x for x in out if x]
    except Exception: pass
    try:
        uid=getattr(u,"id",None); fld="keyword" if hasattr(Keyword,"keyword") else "text"
        for k in db.query(Keyword).filter(Keyword.user_id==uid).all():
            t=(getattr(k,fld,None) or "").strip(); 
            if t: out.append(t)
    except Exception: pass
    return out
def norm(s:str)->str: 
...
def wrap(url:str)->str:
    ...
    import urllib.parse as up; return f"{AFFILIATE_PREFIX}{up.quote(url,safe='')}"
def usd_range(mn,mx,cur):
    ...
    f=lambda v: round(float(v)*rate,2) if v is not None else None
    umin,umax=f(vmin),f(vmax)
...
def get_or_create_job(db, *, source:str, source_id:str, title:str, desc:str,
    ...
    """Upsert στον πίνακα job βάσει (source, source_id). Επιστρέφει job.id"""
    try: db.rollback()
    except Exception: pass
    ...
    row.url = url
    row.proposal_url = proposal_url or url
    row.original_url = original_url or url
    row.budget_min = bmin
    row.budget_max = bmax
    row.budget_currency = (currency or "USD")[:16]
    row.matched_keyword = matched
    row.posted_at = posted_at or now_utc()
    db.commit()
    return int(row.id)
...
async def fetch_skywalker()->List[Dict]:
    ...
    out=[]
    for it in items:
        title=norm("".join(re.findall(r"<title>(.*?)</title>", it, flags=re.S)))
        ...
        out.append({
            ...
            "title":title,"desc":desc,"url":link,"proposal":link,"original":link,
            "bmin":None,"bmax":None,"cur":None,"posted":when or now_utc(),
        })
    return out

async def fetch_freelancer(queries:List[str])->List[Dict]:
    if not FEED_FREELANCER or not queries: return []
    out=[]; base={"limit":30,"compact":"true","user_details":"true","job_details":"true","full_description":"true"}
    async with httpx.AsyncClient(timeout=HTTP_TIMEOUT) as client:
        for q in queries:
            try:
                r=await client.get(FREELANCER_API, params={**base,"query":q}); r.raise_for_status(); data=r.json()
            except Exception as e:
                log.warning("Freelancer fetch failed (%s): %s", q, e); continue
            for p in (data.get("result") or {}).get("projects") or []:
                pid=str(p.get("id"))
                title=norm(p.get("title") or ""); desc=norm(p.get("description") or "")
                ...
                when=datetime.fromtimestamp(int(ts), tz=UTC) if ts else now_utc()
                url=f"https://www.freelancer.com/projects/{pid}"
                out.append({
                    "source":"Freelancer","source_id":pid,
                    "title":title,"desc":desc,"url":url,
                    "proposal":wrap(url),"original":url,
                    "bmin":bmin,"bmax":bmax,"cur":cur,"posted":when,
                })
    return out
...
def dedup_prefer_affiliate(items:List[Dict])->List[Dict]:
    ...
    for it in items:
        ...
        cur=seen.get(key)
        if not cur: seen[key]=it
        else:
            a = bool(it.get("proposal")) and it["proposal"]!=it["url"]
            a0 = bool(cur.get("proposal")) and cur["proposal"]!=cur["url"]
...
async def process_user(db, user, items)->int:
    ...
    for it in matches:
        ...
        jid = get_or_create_job(
            ...
            bmin=it.get("bmin"), bmax=it.get("bmax"), currency=it.get("cur"),
            matched=it.get("_matched"), posted_at=it.get("posted"),
        )
        # avoid duplicate sends per user
        try:
            db.rollback()
        except Exception:
            pass
        ...
        # text
        raw=None; usd=None
        if it.get("bmin") is not None or it.get("bmax") is not None:
            bmin="" if it.get("bmin") is None else str(it.get("bmin"))
            bmax="" if it.get("bmax") is None else str(it.get("bmax"))
            raw=f"{bmin}-{bmax} {it.get('cur')}".strip("- ")
            try:
                rate=float(FX_RATES.get((it.get('cur') or 'USD').upper(),1.0))
                umin=round(float(it.get('bmin'))*rate,2) if it.get('bmin') is not None else None
                umax=round(float(it.get('bmax'))*rate,2) if it.get('bmax') is not None else None
                if umin is None and umax is None: usd=None
                elif umin is None: usd=f"~${umax}"
                elif umax is None: usd=f"~${umin}"
                else: usd=f"~${umin}–{umax}"
        ...
        from_time = it.get("posted") or now_utc()
        lines.append(f"⏱️ {age(from_time)}")
        text="\n".join(lines)

        url_buttons=[("📨 Proposal", it.get("proposal") or it["url"]),
...
async def worker_cycle():
    ...
    try: users=list(db.query(User).all())
        ...
        log.warning("DB users read failed: %s", e); db.close(); return
    ...
    try:
        ...
        if FEED_FREELANCER:
            ...
            for u in users:
                if user_active(u):
                    for k in _kws(db,u):
                        if k: all_k.add(k.strip())
            fr = await fetch_freelancer(sorted(all_k)[:20])
        items = sky + fr
    except Exception as e:
