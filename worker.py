...
def fetch_all(keywords_query: Optional[str] = None) -> List[Dict]:
    ...
    if PLATFORMS.get("peopleperhour"):
        try:
            # keywords_query: comma-separated string ή ήδη λίστα
            if isinstance(keywords_query, str):
                kw_list = [k.strip() for k in keywords_query.split(",") if k.strip()]
            else:
                kw_list = keywords_query or []
            items_pph = pph.get_items(kw_list)
            for i in items_pph:
                i.setdefault("source", "peopleperhour")
                url = i.get("url") or i.get("original_url")
                if url:
                    i.setdefault("original_url", url)
                    i.setdefault("url", url)
                    i.setdefault("proposal_url", url)
                i.setdefault("affiliate", False)
                out.append(i)
        except Exception:
            pass
    ...
    # --- Placeholder platforms (malt, workana, etc) ---
    try:
        if PLATFORMS.get("malt"):
            out += ph.fetch_malt()
        if PLATFORMS.get("workana"):
            out += ph.fetch_workana()
        if PLATFORMS.get("wripple"):
            out += ph.fetch_wripple()
        if PLATFORMS.get("toptal"):
            out += ph.fetch_toptal()
        if PLATFORMS.get("twago"):
            out += ph.fetch_twago()
        if PLATFORMS.get("freelancermap"):
            out += ph.fetch_freelancermap()
        if PLATFORMS.get("younojuno") or PLATFORMS.get("yunoJuno") or PLATFORMS.get("yuno_juno"):
            out += ph.fetch_yunojuno()
        if PLATFORMS.get("worksome"):
            out += ph.fetch_worksome()
        if PLATFORMS.get("codeable"):
            out += ph.fetch_codeable()
        if PLATFORMS.get("guru"):
            out += ph.fetch_guru()
        if PLATFORMS.get("99designs"):
            out += ph.fetch_99designs()
        if PLATFORMS.get("jobfind"):
            out += ph.fetch_jobfind()
        if PLATFORMS.get("kariera"):
            out += ph.fetch_kariera()
        if PLATFORMS.get("careerjet"):
            out += ph.fetch_careerjet()
    except Exception:
        pass
