...
- Welcome card with HTML parse_mode (no raw <b>/<code> showing)
- Big inline buttons (Add Keywords, Settings, Help, Saved, Contact, Admin) with working callbacks
- Shows Trial Start / Trial Ends / License Until in the welcome/settings card
...
- Admin controls:
    ...
    /broadcast <text>
    /feedstatus (alias /feedstats) — 24h per-platform counters
- User → Admin extension flow:
    /extend — sends a request to admin; admin Approve/Decline via inline buttons
- Automatic reminder 24h before end (trial or license) via JobQueue repeating task
"""
...
APPROVAL_EXTRA_DAYS = int(os.getenv("APPROVAL_EXTRA_DAYS", "10"))  # days added on admin Approve
...
def _get_or_create_user(db, tg_id: int):
    ...
    if hasattr(u, "license_until") and getattr(u, "license_until") is None:
...
def _effective_expiry(u) -> Optional[datetime]:
    """License has priority, else trial end."""
    lic = getattr(u, "license_until", None)
    if lic:
        return lic
    return getattr(u, "trial_ends", None)
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Welcome block (HTML)
    await update.message.reply_text(WELCOME_HEAD, reply_markup=main_menu_keyboard(), parse_mode=ParseMode.HTML, disable_web_page_preview=True)

    # Show settings card with dates under the welcome
    if db_available():
        db = SessionLocal()
        try:
            u = _get_or_create_user(db, update.effective_user.id)
            kws = [_get_kw_text(k) for k in _list_keywords(db, u)]
    ...
    try:
        ...
        if not set_ok:
            for uf in ("id", "user_id", "pk"):
                ...
                if set_ok: break
        if hasattr(k, "created_at") and getattr(k, "created_at") is None:
            setattr(k, "created_at", now_utc())
...
async def keywords_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if kws:
            ...
            for k in kws:
                shown.append(_get_kw_text(k))
                lines.append(f"• <code>{_kw_id(k)}</code> — {_get_kw_text(k)}")
...
async def delkeyword_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("Usage: <code>/delkeyword &lt;word&gt;</code> or <code>/delkeyword &lt;id&gt;</code>", parse_mode=ParseMode.HTML)
        return
...
# ---------- User extension request ----------

async def extend_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """User asks for extension; notify admin with Approve/Decline buttons."""
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Approve +{} days".format(APPROVAL_EXTRA_DAYS), callback_data=f"adm:approve:{user_id}")],
...
async def users_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for u in users[:200]:
            ...
            active, blocked = _active_blocked(u)
            lines.append(
                f"• <code>{_display_id(u)}</code> — kw:{len(kws)} | "
...
async def grant_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        await update.message.reply_text("Usage: <code>/grant &lt;telegram_id&gt; &lt;days&gt;</code>", parse_mode=ParseMode.HTML)
        return
    if not db_available():
        await update.message.reply_text("DB not available.")
        return
    ...
    try:
        ...
        if hasattr(u, "license_until"):
            setattr(u, "license_until", _extend_days(lic, days))
        ...
        await update.message.reply_text(
            "✅ Granted access.\n"
            f"user: <code>{_display_id(u)}</code>\n"
...
async def feedstatus_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if SessionLocal is None or JobSent is None:
        await update.message.reply_text("DB not available in this environment."); return
    db = SessionLocal()
    try:
        ...
        if not counts:
            await update.message.reply_text("📊 No sent jobs in the last 24h."); return
...
async def button_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "act:add":
        ...
        try:
            ...
            # extend license or trial
            if hasattr(u, "license_until"):
                setattr(u, "license_until", _extend_days(getattr(u, "license_until", None), APPROVAL_EXTRA_DAYS))
            elif hasattr(u, "trial_ends"):
                setattr(u, "trial_ends", _extend_days(getattr(u, "trial_ends", None), APPROVAL_EXTRA_DAYS))
            if hasattr(u, "active"): setattr(u, "active", True)
            if hasattr(u, "blocked"): setattr(u, "blocked", False)
            db.commit()
            await q.message.reply_text(f"✅ Approved. Extended {APPROVAL_EXTRA_DAYS} day(s) for {target}.")
            try:
                await context.bot.send_message(chat_id=target, text=f"✅ Approved. Your access was extended by {APPROVAL_EXTRA_DAYS} day(s).")
            except Exception: pass
        finally:
            db.close()
                    ...
                    try:
                        await context.bot.send_message(
                            ...
                            text=f"⏳ Reminder: your access expires on <b>{_fmt(exp_utc)}</b>. "
                                 "Use /extend to request extra days.",
                            parse_mode=ParseMode.HTML
                        )
                    except Exception:
                        pass
                ...
                if admin_id_env:
                    try:
                        await context.bot.send_message(
                            ...
                            text=f"⏳ User <code>{_display_id(u)}</code> expires on <b>{_fmt(exp_utc)}</b>.",
                            parse_mode=ParseMode.HTML
                        )
                    except Exception:
                        pass
...
def build_application() -> Application:
    ...
    # schedule daily expiration check (first run in 60s)
    app_.job_queue.run_repeating(check_expirations, interval=24 * 3600, first=60)
