...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    with get_session() as s:
        ...
        try:
            ...
            s.execute(_t("UPDATE \"user\" SET trial_end=COALESCE(trial_end, NOW()) + (:days || ' days')::interval WHERE id=:id")
    ...
    await update.effective_chat.send_message(
        HELP_EN + help_footer(STATS_WINDOW_HOURS, admin=is_admin_user(update.effective_user.id)),
        parse_mode=ParseMode.HTML,
...
async def selftest_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rates = load_fx_rates(os.getenv("FX_USD_RATES", "")) if "utils_fx" else {}
    ccy="USD"
    usd_min = to_usd(budget_min, ccy, rates) if rates else budget_min
    usd_max = to_usd(budget_max, ccy, rates) if rates else budget_max
    job_text = (
        f"<b>{job_title}</b>\n"
        f"  <b>Budget:</b> {budget_min:.1f}–{budget_max:.1f} {ccy} (≈ ${usd_min:.0f}–${usd_max:.0f})\n"
        f"  <b>Source:</b> {source}\n"
...
async def reminder_job(context: ContextTypes.DEFAULT_TYPE):
    # Send reminder 24h before trial_end to active, not blocked and not yet reminded
    try:
        with get_session() as s:
            rows = s.execute(_t('''
                ...
                WHERE is_active=true AND is_blocked=false
                  ...
                  AND trial_end > NOW()
                  AND trial_end <= NOW() + INTERVAL '24 hours'
                  AND COALESCE(trial_reminder_sent,false)=false
            ''')).fetchall()
            for uid, tid, tend in rows:
                try:
                    await context.bot.send_message(
                        chat_id=tid,
                        text=f"⏰ Υπενθύμιση: Η δοκιμή σου λήγει σε 24 ώρες (UTC: {tend}). Αν θέλεις συνέχεια πρόσβαση, πες μου για επέκταση.",
                        parse_mode="HTML"
                    )
        ...
        logging.getLogger(__name__).warning("reminder_job error: %s", e)
...
def build_application() -> Application:
    ...
    app.add_handler(CallbackQueryHandler(cb_mainmenu))
        try:
        app.job_queue.run_repeating(reminder_job, interval=60*60, first=60*5)
    except Exception:
        pass
    return app
