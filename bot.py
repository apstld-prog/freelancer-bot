...
def _kwexpr() -> str: return 'COALESCE(keyword, value)'
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    with get_session() as s:
        ...
        try:
            ...
            s.execute(_t('UPDATE "user" SET trial_end=COALESCE(trial_end, NOW() AT TIME ZONE \\'UTC\\') + INTERVAL :days WHERE id=:id').bindparams(days=f"{TRIAL_DAYS} days"), {"id": u.id})
