...
async def job_action_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Save -> insert into DB (create table if missing, avoid duplicates), then delete the message.
    Delete -> delete the message (or at least strip buttons). No alerts to the user.
    """
    q = update.callback_query
    ...
    if data == "job:save":
        ...
        # delete message or at least clear the buttons
        try:
            if msg:
                await msg.delete()
            else:
