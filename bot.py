...
try:
    from db import (
        SessionLocal as _S, User as _U, Keyword as _K, Job as _J, JobSent as _JS, init_db as _init_db
    )
...
HELP_TEXT = (
    ...
    "<code>{step1}</code>, <code>{step2}</code>, <code>{step3}</code>, <code>{budgettime}</code>, <code>{portfolio}</code>, <code>{name}</code>.\n"
...
def _settings_card(u, kws:List[str]) -> str:
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            pass
        await update.message.reply_text(_settings_card(u, kws), parse_mode=ParseMode.HTML, disable_web_page_preview=True)
    finally:
        try: db.close()
...
async def feedstatus_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        ordered = [
            "99designs","Careerjet","Codeable","Freelancer","Guru","JobFind","Kariera","Malt",
...
def build_application() -> Application:
    ...
    # === Βάλε εδώ ΟΛΟΥΣ τους υπόλοιπους handlers σου όπως τους έχεις ===
    # (addkeyword, keywords, delkeyword, whoami, contact/admin chat, grant/block/unblock/broadcast, buttons, κ.λπ.)
