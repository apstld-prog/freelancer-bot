...
from utils_fx import load_fx_rates
...
def _compose_budget_line(it: Dict, fx: Dict) -> str | None:
    ...
    if cur and cur != "USD":
        if bmin is not None and umin is None:
            umin = _cur_to_usd(bmin, cur, fx)
        if bmax is not None and umax is None:
            umax = _cur_to_usd(bmax, cur, fx)

    if bmin is not None and bmax is not None:
        native = f"{_fmt(bmin)}–{_fmt(bmax)} {cur or 'USD'}"
    else:
        native = f"{_fmt(bmin if bmin is not None else bmax)} {cur or 'USD'}"

    usd_part = ""
    if cur and cur != "USD" and (umin is not None or umax is not None):
        if umin is not None and umax is not None:
            usd_part = f" (~ {_fmt(umin)}–{_fmt(umax)} USD)"
        else:
            usd_part = f" (~ {_fmt(umin if umin is not None else umax)} USD)"
...
def _compose_text(it: Dict, fx: Dict) -> str:
    ...
    age = _posted_ago(it)
    if age: parts.append(f"🕓 Posted: {age}")

    return "\n".join(parts)

def _kb(proposal_url: str, original_url: str) -> InlineKeyboardMarkup:
    row1 = [
...
async def _send(bot: Bot, uid: int, text: str, kb: InlineKeyboardMarkup):
    await bot.send_message(chat_id=uid, text=text, parse_mode=ParseMode.HTML, reply_markup=kb, disable_web_page_preview=True)

async def _run_async():
    s = get_session()
    token = os.getenv("TELEGRAM_BOT_TOKEN", "").strip()
    if not token:
        log.error("Missing TELEGRAM_BOT_TOKEN"); return
    ...
    for it in items:
        try:
            ...
            for uid in receivers:
                ...
                await _send(bot, uid, msg, _kb(prop or orig, orig or prop))
                sent += 1
                await asyncio.sleep(0.2)
        except Exception as e:
            log.warning("compose/send skipped: %s", e)
